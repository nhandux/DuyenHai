...
function product($act, $typeFunc, $product_id, $product_menu_id, $name, $img, $img_note, $product_keys, $price, $model, $train, $sale, $comment, $content, $upload_img_id, $is_active, $hot, $pin, $created_time, $title, $description, $keywords, $error) {
	global $db, $tth;
				...
				<div class="table-respon">
					<form action="<?=$act?>" method="post" enctype="multipart/form-data">
						...
						<table class="table table-hover">
							...
							<tr>
								<td><label>Trọng lượng: (gram)</label></td>
							...
							<?php echo $train; ?>
                                ...
                                <?php
									...
									$rows = $db->select();
										...
										foreach ($rows as $row) {
											...
											<option value="<?= $row['product_menu_id'] ?>" <?php if($row['product_menu_id'] == $train)
								...
								<!-- <input class="form-control" type="text" name="model" maxlength="255" value="<?=stripslashes($model)?>">
 -->
								</td>
								
							</tr>
							<tr>
								<td class="ver-top"><label>Đơn vị tính:</label></td>
								<td colspan="3">
